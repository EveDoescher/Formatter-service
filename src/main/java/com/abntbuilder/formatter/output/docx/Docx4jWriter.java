...
import com.abntbuilder.formatter.engine.model.output.*;
...
public class Docx4jWriter implements DocxWriter {
    ...
    private void writeTable(WordprocessingMLPackage wordPackage, DocxTableBlock tableBlock, long textWidthTwips) {
        ...
        for (List<com.abntbuilder.formatter.engine.model.output.DocxTableCell> row : tableBlock.rows()) {
    ...
    private TblPr createTableProperties(DocxTableBlock tableBlock, long textWidthTwips) {
        ...
        if (tableBlock.borderStyle() == com.abntbuilder.formatter.engine.model.output.TableBorderStyle.CLOSED) {
    ...
    private PPr createParagraphProperties(
        ...
        long firstLineTwips = MeasurementConverter.centimetersToTwips(styleRule.firstLineIndentCm());
        if (firstLineTwips < 0) {
            indentation.setHanging(BigInteger.valueOf(-firstLineTwips));
        } else {
            indentation.setFirstLine(BigInteger.valueOf(firstLineTwips));
        }
    ...
    private void writeFootnoteReferenceBlock(WordprocessingMLPackage wordPackage, com.abntbuilder.formatter.engine.model.output.DocxFootnoteReferenceBlock fnBlock, ListNumIds listNumIds, long textWidthTwips) {
        try {
            ...
            for (com.abntbuilder.formatter.engine.model.output.DocxFootnoteContent fn : fnBlock.footnotes()) {
                ...
                for (com.abntbuilder.formatter.engine.model.output.DocxRun docxRun : fn.contentRuns()) {
                    R run = objectFactory.createR();
                    run.setRPr(buildRunProperties(docxRun.baseStyle(), docxRun.formatting()));
                    Text t = objectFactory.createText();
    ...
    private ListNumIds createListNumIds(WordprocessingMLPackage wordPackage) {
        ...
        // Additional list types: alphabetic lower, alphabetic upper, roman lower, roman upper
        int[][] extras = {
            {2, 3, org.docx4j.wml.NumberFormat.LOWER_LETTER.ordinal()},   // abstractId 2, numId 3
            {3, 4, org.docx4j.wml.NumberFormat.UPPER_LETTER.ordinal()},   // abstractId 3, numId 4
            {4, 5, org.docx4j.wml.NumberFormat.LOWER_ROMAN.ordinal()},    // abstractId 4, numId 5
            {5, 6, org.docx4j.wml.NumberFormat.UPPER_ROMAN.ordinal()},    // abstractId 5, numId 6
        };
        org.docx4j.wml.NumberFormat[] extraFormats = {
            org.docx4j.wml.NumberFormat.LOWER_LETTER,
            org.docx4j.wml.NumberFormat.UPPER_LETTER,
            org.docx4j.wml.NumberFormat.LOWER_ROMAN,
            org.docx4j.wml.NumberFormat.UPPER_ROMAN
        };
        for (int i = 0; i < extraFormats.length; i++) {
            Numbering.AbstractNum abstractExtra = objectFactory.createNumberingAbstractNum();
            abstractExtra.setAbstractNumId(BigInteger.valueOf(i + 2));
            Lvl lvlExtra = objectFactory.createLvl();
            lvlExtra.setIlvl(BigInteger.ZERO);
            NumFmt fmtExtra = objectFactory.createNumFmt();
            fmtExtra.setVal(extraFormats[i]);
            lvlExtra.setNumFmt(fmtExtra);
            Lvl.LvlText lvlTextExtra = objectFactory.createLvlLvlText();
            lvlTextExtra.setVal("%1)");
            lvlExtra.setLvlText(lvlTextExtra);
            Lvl.Start startExtra = objectFactory.createLvlStart();
            startExtra.setVal(BigInteger.ONE);
            lvlExtra.setStart(startExtra);
            abstractExtra.getLvl().add(lvlExtra);
            numbering.getAbstractNum().add(abstractExtra);

            Numbering.Num numExtra = objectFactory.createNumberingNum();
            numExtra.setNumId(BigInteger.valueOf(i + 3));
            Numbering.Num.AbstractNumId aoidExtra = objectFactory.createNumberingNumAbstractNumId();
            aoidExtra.setVal(BigInteger.valueOf(i + 2));
            numExtra.setAbstractNumId(aoidExtra);
            numbering.getNum().add(numExtra);
        }
    ...
    private void writeListItem(
        ...
        int id = switch (listItem.listType()) {
            case ORDERED         -> listNumIds.orderedNumId();
            case UNORDERED       -> listNumIds.unorderedNumId();
            case ALPHABETIC_LOWER -> 3;
            case ALPHABETIC_UPPER -> 4;
            case ROMAN_LOWER      -> 5;
            case ROMAN_UPPER      -> 6;
        };
